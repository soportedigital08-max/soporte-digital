      ...
      <main className="flex-1 bg-primary-900">
        <article className="py-24">
          <div className="container max-w-3xl mx-auto">
            ...
            <h1 className="text-h1 text-white mt-2">{articulo.titulo}</h1>
            <p className="text-body-lg text-primary-200 mt-6">{articulo.extracto}</p>
            ...
            <div className="mt-12 flex flex-col gap-6">
              {articulo.bloques.map((b, i) => {
                ...
                if (b.tipo === "ul")
                  return (
                    <ul key={i} className="text-body-lg text-primary-200 list-disc list-inside flex flex-col gap-2">
                      {b.items.map((it, j) => <li key={j}>{it}</li>)}
                    </ul>
                  );
                return <p key={i} className="text-body-lg text-primary-200">{b.texto}</p>;
              })}
            </div>
          </div>
        </article>
        ...
        <section className="py-24 text-center bg-primary-800/40">
          <div className="container">
            ...
            <p className="text-body-lg text-primary-200 mt-4">Te lo resolvemos.</p>
            <div className="mt-8">
